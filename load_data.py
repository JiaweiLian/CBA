...
class MeanProbExtractor_yolov2(nn.Module):
    ...
    def forward(self, output):
        output = get_region_boxes(output, self.conf_thres, self.model.num_classes, self.model.anchors, self.model.num_anchors)[0]
        # print(output)
        # print("##################################################")
        ...
        if len(output) > 0:
            ...
            # print(output)
            # print("##################################################")
            label = output[:, 6]  # Extract label column, e.g. tensor([ 0.,  0.,  0.,  0., 14.])
            index = label == 0.0  # Labels are plane, e.g. tensor([ True,  True,  True,  True, False])
            output = output[index]  # Predictions of plane
            # print(output)
            # print("##################################################")
            probs = output[:, 4]
            sum = 0.0
            for prob in probs:
                sum = sum + prob
            if len(probs) > 0:
                mean_conf = sum/len(probs)
            else:
                mean_conf = torch.tensor(float(0))
            # probs = torch.tensor(output[:, 4])
            # mean_conf = torch.mean(probs)  # The mean value of confs
            # print(mean_conf)
            # print("##################################################")
...
class MeanProbExtractor_yolov5(nn.Module):
    ...
    def forward(self, YOLOoutput):  # YOLOoutput: torch.Size([batch, 64512, 20])
        ...
        # print(output)
        if len(output) > 0:
            mean_conf = torch.mean(output[:, 4])  # The mean value of confs
        else:  # Without objects
            mean_conf = torch.tensor(float(0))
        # print(mean_conf)
        return mean_conf
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        # target_y = target_y - 0.08 * (target_size / current_patch_size / 3 * 5)
        #########################################################################################
...
class PatchTransformer_APPA_upper(nn.Module):
    """PatchTransformer: transforms batch of patches
    ...
    def __init__(self):
        super(PatchTransformer_APPA_upper, self).__init__()
        self.min_contrast = 0.8
        self.max_contrast = 1.2
        self.min_brightness = -0.1
        self.max_brightness = 0.1
...
class PatchTransformer_A3(nn.Module):
    """PatchTransformer: transforms batch of patches
    ...
    def __init__(self):
        super(PatchTransformer_A3, self).__init__()
        self.min_contrast = 0.8
        self.max_contrast = 1.2
        self.min_brightness = -0.1
        self.max_brightness = 0.1
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        ############################################################################################
        scale = target_size / current_patch_size * 4.5  # patch outside targets
        # scale = target_size / current_patch_size  # patch on targets
        ############################################################################################
