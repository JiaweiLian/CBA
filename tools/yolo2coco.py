...
parser.add_argument('--root_dir', default='clean/labels-yolo', type=str,
                    help="root path of images and labels, include ./images and ./labels and classes.txt")
parser.add_argument('--save_path', type=str, default='aircraft.json',
...
def yolo2coco(arg):
    ...
    originLabelsDir = os.path.join(root_path, 'labels-yolo')
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        with open(os.path.join(originLabelsDir, txtFile), 'r') as fr:
            ...
            for label in labelList:
                ...
                x = float(label[1]) * 1024
                y = float(label[2]) * 1024
                w = float(label[3]) * 1024
                h = float(label[4]) * 1024
    ...
    if arg.random_split or arg.split_by_file:
        ...
        with open(json_name, 'a') as f:  # 'a':文件不存在则创建文件
            json.dump(dataset, f)
            print('Save annotation to {}'.format(json_name))
...
if __name__ == "__main__":
    yolo2coco(arg)
# python yolo2coco.py --root_dir '/home/mnt/ljw305/DOTA_devkit_YOLO/datasets/DOTA1.0/val_split' --save_path 'instances_val2017.json'
