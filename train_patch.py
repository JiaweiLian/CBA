        ...
        for epoch in range(n_epochs):
            ...
            for i_batch, (img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}', total=self.epoch_length):
                with autograd.detect_anomaly():
                    ...
                    # print(p_img_batch, p_img_batch.size())  # torch.Size([1, 3, 1024, 1024])
                    ...
                    if i_batch + 1 >= len(train_loader):
                        print('\n')
                    del adv_batch_t, output, extracted_prob, det_loss, p_img_batch, nps_loss, tv_loss, loss
...
def main():
    ...
    # print(sys.argv)  # ['train_patch.py', 'paper_obj']
    # trainer = PatchTrainer(sys.argv[1])
    trainer = PatchTrainer('yolov5x')
    trainer.train()
